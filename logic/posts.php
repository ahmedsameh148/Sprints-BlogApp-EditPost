...
function getPost($post_id) {
    ...
    $types = '';
    $vals = [];
    $sql = addWhereConditions($sql, null, null, null, null, $types, $vals);

    $post =  getRow($sql, $types, $vals);
    $post['comments'] = getPostCommentsCount($post['id']);
    $post['tags'] = getPostTags($post_id);

    return $post;
}

function editPost($request, $image)
{
    $post_id = $request['id'];
    $sql = "UPDATE posts SET title = ?, content = ?, image = ?, publish_date = ?, category_id = ?
            WHERE id = ?;";
    editData($sql, 'ssssii', [
        $request['title'],
        $request['content'],
    ...
    editData("DELETE FROM post_tags WHERE post_id = $post_id");
    if (isset($request['tags'])) {
        foreach ($request['tags'] as $tag_id) {
            addData(
                "INSERT INTO post_tags (post_id,tag_id) VALUES (?,?)",
